...
while True:
    ...
    for i, obj in enumerate(results.xyxy[0]):
        ...
        # TODO: 출력 바운딩박스 크기 조절
        x1_scaled = int(x1 * frame_w / input_w)
        y1_scaled = int(y1 * frame_h / input_h)
        x2_scaled = int(x2 * frame_w / input_w)
        y2_scaled = int(y2 * frame_h / input_h)

        # OpenCV를 이용해서 해당 좌표에 사각형과 text를 출력
        cv2.rectangle(frame, (x1_scaled, y1_scaled), (x2_scaled, y2_scaled), (0, 255, 0), 2)
        cv2.putText(frame, label_names[str(cls)], (x1_scaled, y1_scaled - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        print(f"Object {i}: {label_names[str(cls)]} at [{x1_scaled}, {y1_scaled}, {x2_scaled}, {y2_scaled}]")
